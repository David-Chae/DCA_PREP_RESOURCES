...
### 6. IPvlan Driver
The ipvlan driver allows you to assign containers their own IP addresses from a parent interface, providing high-performance networking by bypassing much of the Docker networking stack. It is particularly useful in scenarios where containers need to appear as physical hosts on the same network as the Docker host.

### Characteristics:
- Assigns a unique IP to each container directly from a subnet.
- Containers can appear as peers on the local physical network.
- Provides better performance by avoiding bridge and NAT overhead.
- Supports both **L2 (bridge mode)** and **L3 (routing mode)** configurations.
- Requires MACVLAN-supporting NIC and suitable configuration at the OS level.

### Use Case:
- Suitable when containers need to communicate directly with external systems on the same physical network.
- Useful in bare-metal environments where traditional bridge networking or NAT is undesirable.
- Ideal for legacy applications that expect a direct network connection.
- Common in high-performance or multi-tenant network environments where IP address management is critical.
...
In this example, each container attached to `my_ipvlan_network` will get an IP from the `192.168.100.0/24` subnet and communicate directly over `eth0`.
...
- [Ipvlan Network](https://docs.docker.com/engine/network/drivers/ipvlan/)
